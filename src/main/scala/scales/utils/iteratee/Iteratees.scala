...
import scalaz6.{IterV, Enumerator, Input, EphemeralStream}
import scalaz6.IterV._
...
trait Iteratees {
  ...
  def onDone[E,A](originalList : List[ResumableIter[E,A]]) : ResumableIterList[E,A] = {
	...
	@inline def add( k : (scalaz6.Input[E]) => scalaz6.IterV[E,(A, scalaz6.IterV[E, _])]) {
	  val d = k(El(e))
	  newl = d :: newl
	  d match {
	...
	while(i.hasNext) {
	  ...
	  case iter@Done(e1, _) =>
		...
		// break on purpose :-()
		e1._2 match {
		  case i @ Cont(k) => add(k.asInstanceOf[(scalaz6.Input[E]) => scalaz6.IterV[E,(A, scalaz6.IterV[E, _])]])
  ...
  def enumToMany[E, A, R]( dest: ResumableIter[A,R])( toMany: ResumableIter[E, EphemeralStream[A]]): ResumableIter[E, R] = {
     ...
     */
    def pumpNext(x: Input[E], toMany: scalaz6.Input[E] => ResumableIter[E,EphemeralStream[A]], k: scalaz6.Input[A] => ResumableIter[A,R] ): ResumableIter[E, R] = {
	  ...
	  val nextContR = nextCont.asInstanceOf[ResumableIter[E,scalaz6.EphemeralStream[A]]]
     ...
     */
    def contk(k: scalaz6.Input[A] => ResumableIter[A,R], i: ResumableIter[A,R], s: () => EphemeralStream[A], toMany: ResumableIter[E, EphemeralStream[A]] ): ResumableIter[E, R] = {
      if (!s().isEmpty) {
	val (ni, ns) = loop(i, s)
	...
	Cont((x: Input[E]) =>
	  x( el = e => {
		...
		val (e1, nextContR) = a
		val nextCont = nextContR.asInstanceOf[ResumableIter[E,scalaz6.EphemeralStream[A]]]
